...
void AIWine::turnBest(int &x, int &y)
{
	...
	if (board->boardSize == 0 ) setSize(15);
...
{
	Cand best = Cand(rootCand[0].point, alpha - 1);
	int value;
	for (int i = 0; i < nRootCand; i++)
	{
...
int AIWine::search(int depth, int alpha, int beta)
{
	...
	if (depth <= 0)
	{
        ...
        if (eval < beta)
        {
            ...
            if ((lastPoint = board->findVCTLastPoint()) != -1 && board->vctSearch(board->who, 0, board->MAX_VCT_DEPTH-board->ply, lastPoint) > 0) return WinScore;
        }
        return eval;
	}
